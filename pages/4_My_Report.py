...
st.set_page_config(
    page_title="My Report — FloodWatch NOLA",
    page_icon="📄",
    layout="wide",
)
...
def _pre_event_checklist(prop_type: str) -> str:
    base = [
        ...
        "- [ ] Check SWBNO pump status at swbno.org and on FloodWatch NOLA",
...
- [ ] Monitor NOAA weather radio or the FloodWatch NOLA app
...
def _generate_report(
    ...
    report = f"""# FloodWatch NOLA — Resilience Report
...
*Generated by FloodWatch NOLA — Educational purposes only.*
...
if submitted:
    ...
    st.download_button(
        ...
        file_name=f"FloodWatch_NOLA_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
        mime="text/plain",
